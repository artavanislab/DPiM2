...
#include "PreciseWriter.hpp" 

PreyByPrey::PreyByPrey(const BaitByPrey &bbp_mat) {
  ...
  for (unsigned int i = 0; i < preyByPrey.size(); ++i) {
    ...
    // row and col sums required because it is a triangular matrix
  }

  {
    double minDouble;
    if (std::numeric_limits<double>::has_denorm) {
      // floating point values less than the "minimum" double
      // potentially platform dependent
      minDouble = std::numeric_limits<double>::denorm_min();
    } else {
      minDouble = std::numeric_limits<double>::min();
    }
    maxScore = ceil(-log(minDouble)); // ceil just makes clipping obvious
  }
  
  precision = 3;
  cout.precision(precision+2); // cout interprets it funny but shows
  // scientific notation

  verbose = bbp_mat.verbose;
  dumpContingency=false;
  return;
}


void PreyByPrey::makePBPmatrix(const BaitByPrey &bbp_mat) {
  if (verbose) {
    cout << "... computing a " << preyRef2Idx.size() << " X "
	 << preyRef2Idx.size() << " prey-by-prey data matrix" << endl;
  }
  ...
  for (unsigned int i = 0; i < bbp_mat.searchByPrey.size(); i++) {
    ...
    for (unsigned int j = 0; j < bbp_mat.searchByPrey[0].size(); j++) {
      ...
      if (bbp_mat.searchByPrey[i][j] > 0) { // pulled down means non-zero tsc!
...
void PreyByPrey::computeHyge() {
  ...
  for (i = 0; i < preyByPrey.size(); ++i) {
	...
	if (dumpContingency) {
	  cout << uniqPreyReferenceVec[i] << "\t" << uniqPreyReferenceVec[j] 
	       << "\t" << A << "\t" << B << "\t" << AB << "\t" << N << "\t";
	}
	if (A > B) {
	  swap(A,B);
...
void PreyByPrey::json(string outFile) {
  ...
  PreciseWriter<rapidjson::StringBuffer> writer(sb, precision);
  //rapidjson::PrettyWriter approximately doubles the disk space
  writer.StartObject();

  // report HGScore -- only the non-zero elements
  const double minScore = pow(10, -precision); // report only elements with score higher than
  writer.String("score");
  writer.StartObject();
