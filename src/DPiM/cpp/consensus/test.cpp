...
void writeJson(const std::string &outFile,
	       ...
	       const stringVector& protList, int precision);
...
int main  (int argc, char* argv[]) {
  ...
  double interactionFraction = 0.8;
  int precision = 3;
  ...
  for (size_t i=0; i < nSheets; ++i) {
    stringVector thisProtList = randomSubset(protList, protList.size() *
					     interactionFraction);
    doubleVectorVector randMat;
    fillRandMat(randMat, allScores, i, thisProtList, interactionProb, mt);

    std::string json = createJson(randMat, thisProtList, precision);
    simpleJsonTest(json.c_str());

    std::string outFile = std::tmpnam(nullptr);
    writeJson(outFile, randMat, thisProtList, precision);
  ...
  Network n(files, nProt);

  std::cout << "testing" << std::endl;
  bool passFlag = true;
  double thresh = pow(10, -precision);
  BOOST_FOREACH(std::string p1, protList) {
    BOOST_FOREACH(std::string p2, protList) {
      if (! p1.compare(p2)) {
	continue;
      ...
      if ( ((std::abs(1 - nMean/mean) > thresh) &&
	    ! (std::abs(nMean) < thresh && std::abs(mean) < thresh))
	   || ((std::abs(1 - nSd/sd) > thresh) &&
	       ! (std::abs(nSd) < thresh && std::abs(sd) < thresh)))
      {
	       ...
	       const stringVector& protList, int precision)
{
  ...
  out << createJson(preyByPreyHyge, protList, precision) << std::endl;
  out.close();
}
...
std::string createJson(const doubleVectorVector& preyByPreyHyge,
		       const stringVector& protList, int precision)
{
  const double minScore = pow(10, -precision); // report only scores higher than ~

  rj::StringBuffer sb;
  PreciseWriter<rj::StringBuffer> writer(sb, precision);
