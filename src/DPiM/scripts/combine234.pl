...

{
	...
	$lookupFile = '/home/glocke/DPiM/dpim4/excelCsv/sid2DateInstrID.tsv';
	if (! -s $lookupFile) {
	    say "creating $lookupFile";
	    $lookupScript = $ENV{DPSCR}."extractDateFromExcelCSV.pl";
	    my $cmd = "$lookupScript $lookupFile";
	    say $cmd;
	    system($cmd);
	}
    }
...
sub getCommandLineOptions {
    ...
    my $usage = "usage: $0 -out output < $defaultString -dp4date ".
	"remakeIfNotSupplied > \n";
    ...
    checkExist('f', $opts{dp4date}) if exists $opts{dp4date};
