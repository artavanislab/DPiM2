...
use Statistics::Descriptive qw(median);
#use Statistics::Basic qw(median); #KJ commented this out because the Basic is no longer avail, added above line
...

{
    ...
    my @cutoffs = ();
    for my $sf (@simFiles) {
	push @cutoffs, compHGScoreCutoff($realFile, $sf, $fdr);
    }
    say "cutoffs <- c(".( join ",", @cutoffs ).")";
	my $stat = Statistics::Descriptive::Full->new();
	$stat->add_data(@cutoffs);
    #my $hgCutoff = $stat->median(\@cutoffs);
    my $hgCutoff = $stat->median();
...
sub compHGScoreCutoff {
    my ($realFile, $simFile, $targetFDR) = @_;

    $targetFDR //= 0.05; # by default look for cutoff for 0.05 FDR
    ...
    my $s = 0;
    my $r;
    while ($FDR < $targetFDR && $s < $#simulated_edges) {
	$r = 0;
