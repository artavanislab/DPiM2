...
source("/home/kli3/DPiM/scripts/rscripts/homebrew.R") # try to not use current user home path
.libPaths(.libPaths()[-1]) # avoid using current user home lib path
...
# temporary skip pfam
collector <- function(egTab, goTab, reacTab, otTab, mbTab) {
    reacTab <- merge(reacTab, egTab, by.x="ENTREZID", by.y="ENTREZID")

    ret <- rbindlist(list(goTab, reacTab[,c(idCol, "REID")],
                          otTab, mbTab))
    ret <- as.data.frame(ret)
    names(ret) <- c(idCol, "ID")
    ret <- subset(ret, ! is.na(ret$ID))
    ret <- subset(ret, ret$ID != "RE:NA")
...
otTable <- function(allEG, otFile) {
	...
	colz <- unique(c(idCol, qw("ENTREZID SYMBOL")))
	...
	ret <- merge(otTab, ot4, by.x="SYMBOL", by.y="GENE")


    return(list(unique(ret[,qw("ENTREZID OpenTargets_ID")]), ot3))
}
...
mbTable <- function(allEG, mbFile) {
    ...
    ret <- merge(mbTab, mb, by.x="SYMBOL", by.y="genesymbol")

    return(list(unique(ret[,qw("ENTREZID mb_ID")]), unique(mb[,qw("mb_ID uniqName")])))
}
...
oneClustTest <- function(cl, goodTerms, nTerms, termNames) {
	...
	#KJ modified this line to allow output all signifcant terms not just top 3
	len <- sum(qvals[x$ix]<0.05)
	bestTerm <- names(x$x[1:len])
...
defaults <- list(
	...
	pfamFile = '/home/kli3/DPiM/augRemap/apmsData/annotation/drome-pfams.txt',
	...
	pfamNameFile = '/home/kli3/DPiM/augRemap/apmsData/annotation/drome-pfams.txt',
	#fbFile = '/home/glocke/DPiM/augRemap/apmsData/annotation/allele_human_disease_model_data_fb_2017_02.txt',
	fbFile = '/home/kli3/DPiM/augRemap/apmsData/annotation/allele_human_disease_model_data_fb_2017_02.txt',
...

{
	...
	#argv = c('all.txt.tidy','all.txt.tidy.hypGeo.termTest',"human")
    if (length(argv) < 2) {
        tmpModes <- modes
...
#clustFile <- 'all.txt.tidy'
#outFile <- 'all.txt.tidy.hypGeo.termTest'
#moode = 'human'
...
if (moode == 'fly') {
	stop("please use fly specific code for annotation purpose.")
    eg.db <- org.Dm.eg.db
    idCol <- "FLYBASE"
} else if (moode == 'human') {
    eg.db <- org.Hs.eg.db
...

{
    ...
    allEG <- select(eg.db, allIDs, c(idCol), keytype=idCol)
	allEG <- unique(allEG) #KJ added this on 03/12/2021
    allEG <- dropRowsWithNaN(allEG, quant=FALSE)
    row.names(allEG) <- allEG[,idCol]
    allEG$ENTREZID <- as.character(allEG$ENTREZID) # convert factor to character
    nativeCl <- clusters
...

{
    ...
    allGO <- select(eg.db, allIDs, c(idCol, "GO"), keytype=idCol)
    ...
    reacNames$MYNAME <- gsub("Homo sapiens: ", "",
                             reacNames$PATHNAME)
	...
	allTerms <- collector(allEG, allGO, allReac, allOt, allMb)
