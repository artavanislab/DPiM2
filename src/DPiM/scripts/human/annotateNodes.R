...
# KJ: this is a direct copy of the human/annotateClusters.R file
...

{
	...
	allEG <- select(eg.db, allIDs, c("SYMBOL", idCol), keytype=idCol)
    allEG <- dropRowsWithNaN(allEG, quant=FALSE)
    write.table(allEG, file="human_symbol_entrez_trans.txt", sep="\t", row.names=F, quote=F)
