...

{
    ...
    my $format = join "\t", ("%s") x 16;
    #my $header = join "\t", qw(EntrezID Symbol freq avgPep clusterI clusterName 
	...
	my $header = join "\t", qw(EntrezID Symbol clusterI clusterName
                               goID goName reactomeID reactomeName
                               pfamID pfamName flybaseDOID flybaseDOIDName OpenTargetsDiseaseID OpenTargetsDiseaseName MetaBaseDiseaseID MetaBaseDiseaseName );
    ...
    my @data = (\@fbgn, \@symbol, \@clusterI, \@clusterName,
        \@goIDs, \@goNames, \@reIDs, \@reNames, \@pfIDs,
        \@pfNames, \@fbIDs, \@fbNames, \@otIDs, \@otNames, \@mbIDs, \@mbNames);
...
sub getCommandLineOptions {
	...
	termscr => $ENV{DPSCR}."/human/annotateNodes.R",
...
sub readTerms {
    ...
    my @cols = qw(ENTREZID ID name);
    my @read;
    readColsRef(\@read, $termFile, \@cols, undef, "\t");
    my @DBs = qw(GO Reactome OpenTargets MetaBase);
    my $termDB;
    my %terms;
    for my $row (@read) {
	my $fbgn = $row->{ENTREZID};
	next unless exists $nodes->{$fbgn};
	if ($row->{ID} =~ /^GO/) {
	    $termDB = 'GO';
	} elsif ($row->{ID} =~ /^RE/) {
