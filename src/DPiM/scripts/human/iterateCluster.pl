...
sub getCommandLineOptions {
	...
	grepedge => $ENV{DPSCR}."/human/findEdgesWithNodes.pl",
	);
...
sub makeMCLReadable {
    ...
    my $cmd = qq[grep -v "^#" $in | grep -v prey  | grep -v node | grep -v Interactor | grep -v protein | grep -v Gene | awk '{print \$1"\\t"\$2"\\t"\$3}' > $out];
    say $cmd;
    system($cmd);
