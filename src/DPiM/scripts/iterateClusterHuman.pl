...
sub getCommandLineOptions {
	...
	grepedge => $ENV{DPSCR}."/secondary/findEdgesWithNodesHuman.pl",
	);
    ...
    my %opts = ();
    GetOptions(\%opts, "in=s", "dir=s", "mode=s", "out=s", "i=s", "istep=f",
	       "imax=f", "maxclust=i", "mcl=s", "grepedge=s");
...
sub makeMCLReadable {
    ...
    my $cmd = qq[grep -v "^#" $in | grep -v prey  | grep -v node | grep -v Interactor | grep -v protein | grep -v Bait| awk '{print \$1"\\t"\$2"\\t"\$3}' > $out];
    say $cmd;
    system($cmd);
