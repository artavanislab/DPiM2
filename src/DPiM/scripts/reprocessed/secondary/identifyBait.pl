...

{
    ...
    my (%prevBait, %rejectMap);
    {
	my @read;
	my @cols = qw(ms_inst_run_id tap_id newbait rejected wonky tap_test );
	readColsRef(\@read, $prevBaitFile, \@cols);
	my $unknown = 'FBgn0000000';
	for my $row (@read) {
	    $prevBait{$row->{ms_inst_run_id}} = $row->{newbait};
	    if ($row->{wonky} || $row->{rejected} || 
		$row->{tap_test} eq 'DISAGREE' || $row->{tap_id} =~ /^FL/ ||
		$row->{tap_id} eq 'FH0000')
	    {
		my $rid = $row->{ms_inst_run_id};
		$prevBait{$rid} = $unknown;
		$rejectMap{$rid} = 'wonky' if $row->{wonky};
		$rejectMap{$rid} = 'rejected' if $row->{rejected};
		$rejectMap{$rid} = 'humanCantAnnotate' 
		    if $row->{tap_test} eq 'DISAGREE';
		$rejectMap{$rid} = 'FL' if $row->{tap_id} =~ /^FL/;
		$rejectMap{$rid} = 'FH0000' if $row->{tap_id} eq 'FH0000';
	    }
	}
    }
    ...
    my %apms;
    readAPMS(\%apms, $in);
    die "found no experiments - maybe you want to use -logp?"
	if 0 == keys %apms;
    ##die Dumper($apms{f11678});
...
sub getCommandLineOptions {
    ...
    my %defaults = (
	prevbait => '/home/glocke/DPiM/augRemap/apmsData/annotation/baitIdentityTable.09-02-2016.tsv',
