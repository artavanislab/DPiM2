...

{
    ...
    my %prevNet;
    networkHashFromEdgeList(\%prevNet, $prevNetFile, undef, 'symmetric');

    my @cols = qw( protein1 protein2 score flySymbol1 flySymbol2 prevNet
	...
	my $flySymbol1 = $annot{$fbgn1}{flySymbol};
	my $flySymbol2 = $annot{$fbgn2}{flySymbol};

	my $prevNet = isTrue(exists $prevNet{$fbgn1}{$fbgn2});

	my $humanSymbol1 = $annot{$fbgn1}{humanSymbol};
	my $humanSymbol2 = $annot{$fbgn2}{humanSymbol};
	...
	$line.="\t";
	$line.= join("\t", $flySymbol1, $flySymbol2, $prevNet, $humanSymbol1, 
...
sub getCommandLineOptions {
    ...
    my %defaults = (
	prevnet => '/home/glocke/DPiM/prevDPIM/DPIM1_scores.r6.07.updateFBgn.tsv',
	);
    my $defaultString = 
...
sub readAnnot {
    ...
    my @cols = qw(fbgn flySymbol clusterI clusterName humanSymbol
    ...
    for my $row (@read) {
	my ($fbgn, $flySymbol, $clusterI, $clusterName, $humanSymbol, 
	...
	$ret->{$fbgn} = { flySymbol=>$flySymbol, humanSymbol=>$humanSymbol,
