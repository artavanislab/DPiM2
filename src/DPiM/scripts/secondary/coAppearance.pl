...
use DpimLib qw(parseAPMS getLineDP4APMS getLineHyperspecAPMS);
...
{
    my $in = $opts{in};
    my %prot = map {$_ => 1} split /,/, $opts{prot};
    my $out = $opts{out};
...
sub getCommandLineOptions {
    ...
    my $usage = "usage: $0 -in apms -prot FBgn1,FBgn2  -out output < ".
	"$modeString -sidout print.search_ids >\n";

    my %opts = ();
    GetOptions(\%opts, "in=s", "out=s", "prot=s", "mode=s", "sidout=s");
