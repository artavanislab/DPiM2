...

{
    ...
    my $humanMode = $opts{mode} eq 'human';
    ...
    my %reference;
    networkHashFromEdgeList(\%reference, $targetFile, 'symm', undef, undef,
			    $humanMode);

    open my $IN, "<", $sourceFile or die "Can't read from $sourceFile. $!";
    my %missing;
    while (my $line = readHS($IN, $humanMode)) {
	$_ = $line;
	my @spl = split;
...
sub getCommandLineOptions {
	...
	"output < $defaultString -addcol  $modeString>\n";
