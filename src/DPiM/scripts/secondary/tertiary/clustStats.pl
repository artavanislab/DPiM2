...

{
    ...
    my @cols = qw( i nClust	ones	twos	5-perc	10-perc	30-perc	modul	connModul	enriched%	enrichedNum);
    say join "\t", @cols;
    for my $f (@clFiles) {
	my $i = getI($f);
	...
	say join "\t", $i, $nClust, $ones, $twos, $perc5, $perc10, $perc30
	    , $modul, $connModul, $enrichF, $enrichC;
    }
}
...
sub getCommandLineOptions {
    ...
    my $usage = "usage: $0 -in txt.list < $defaultString >\n";

    my %opts = ();
    GetOptions(\%opts, "in=s", "mod=s", "term=s");
