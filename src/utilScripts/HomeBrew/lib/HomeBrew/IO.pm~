...
# /--
# readCols(file, @cols, $wholeLine, $delim, $checkQuotes) = @ret; $ret[i] = hash ref keyed by @cols
# readColsRef(\@ret, ...); as above.
# readCols2(file, @cols, $wholeLine, $delim, $checkQuotes) = %ret; $ret[$col] = array ref
# readCols2Ref(\%values, ...) as above but pass empty \%ret
# \_.> if whole line $wholeLine is defined, it names a column that holds
####   the whole (chomped) line of the file
# readColsHash(file, @cols, $delim, $checkQuotes)
# readColsHashRef(\%ret, ...)
# readColsMultiHashRef(\%ret, ...)
...
# readHeader($file, \$raw, $delim, $checkQuotes) return an array of all the columns listed in the header.  second arg optionally gets set to the raw string header
...
sub lineSplitter {
    my ($checkQuotes, $delim) = @_;

    my $ret;
    if ($delim && $checkQuotes) {
	$ret = sub {
	    my $line = shift;
	    chomp $line;
	    my @ret = quotewords($delim, 'keep', $line);
...
sub readColsRef {
    my ($ret, $f, $cols, $wholeLine, $delim, $checkQuotes) = @_;
    ...
    my $splitter = lineSplitter($checkQuotes, $delim);
...
# hash refs
sub readCols2Ref {
    my ($ret, $f,$colsA, $wholeLine, $delim, $checkQuotes) = @_;
    # verify input
...
sub readColsHash {
    my %ret;
    readColsHashRef(\%ret,@_);
    return %ret;
}


# read two columns from a file.  make a hash using one column as key and the 
#   other as value.
# BUYER BEWARE: if the same key appears more than once, only the last value 
#  will be retained - such disagreements cause warnings
sub readColsHashRef {
    my ($ret, $f, $colsA, $delim, $checkQuotes) = @_;
    # verify input
    die "readCols2Ref: first arg (return value) must be empty hash ref\n" 
	unless ref($ret) eq "HASH" && scalar(keys(%$ret)) == 0;
    die "readCols2Ref: third arg (column list) must be an array ref with two elements\n"
	unless ref($colsA) eq "ARRAY" && @$colsA==2;

    my ($key, $val) = @$colsA;

    my @read;
    readColsRef(\@read, $f, $colsA, undef, $delim, $checkQuotes);
    for my $row (@read) {
	warn "ret->{$row->{$key}} = $ret->{$row->{$key}} and *$row->{$val}*\n"
	    if exists $ret->{$row->{$key}} && 
	    $row->{$val} ne $ret->{$row->{$key}};
...
sub readColsMultiHash {
    my %ret;
    readColsMultiHashRef(\%ret,@_);
    return %ret;
}


# read two columns from a file.  make a hash using one column as key and the 
#   other as value, *allowing multiple values*
# values are also permitted to have multiple keys pointing to them
# ret{keyCol} = {valCol1=>1, valCol2=>2, }
sub readColsMultiHashRef {
    my ($ret, $f, $colsA, $delim, $checkQuotes) = @_;
    # verify input
    die "readCols2Ref: first arg (return value) must be empty hash ref\n" 
	unless ref($ret) eq "HASH" && scalar(keys(%$ret)) == 0;
...
sub readColRef {
    my ($ret, $file, $col, $delim, $checkQuotes) = @_;
...
sub readFastaRef {
	...
	push(@$desc, $seqobj->desc()) if defined $desc;
    }
...
# ...
sub read2DArray {
    my ($ret, $file, $delim, $checkQuotes) = @_;
...
sub readHeader {
    my ($file, $raw, $delim, $checkQuotes) = @_;
    ...
    $$raw = $_ if defined $raw && ref($raw) eq 'SCALAR';
    return @spl;
}
